...
export async function GET() {
  ...
  await connectDb()
  const monthdata = await Location.aggregate([
    {
      $match: {
        createdAt: {
          $lte: new Date(),
  ...
  const yeardata = await Location.aggregate([
    {
      $match: {
  ...
  const todaydata = await Location.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfDay,
  ...
  const weekdata = await Location.aggregate([
    {
      $match: {
        createdAt: {
          $gte: startOfWeek,
  ...
  const yesterdaydata = await Location.aggregate([
    {
      $match: {
        createdAt: {
          $gte: yesterday,
